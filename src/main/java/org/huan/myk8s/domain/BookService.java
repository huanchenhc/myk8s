...
import org.huan.myk8s.elasticsearch.BookProductMapperUtil;
...
public class BookService {
	...
	public BookDTO retrieveBookID(String id, boolean withCache) {
		if(withCache && couchbaseCollection.exists(id).exists()) {
			GetResult getResult = couchbaseCollection.get(id);
			BookDTO book = getResult.contentAs(BookDTO.class);
			if (book != null) {
		...
		// access database
			...
			if (bookAtDB != null) {
				if(withCache) {
					...
					couchbaseCollection.upsert(id, JsonObject.create().put("id", bookAtDB.getId())
							.put("title", bookAtDB.getTitle())
							.put("description", bookAtDB.getDescription()));
				}
	...
	public void createBook(final BookDTO book) {
		...
		String createProductIndex = productSearchService.createProductIndex(BookProductMapperUtil.toProduct(book));
		logger.info("create book in elastic " + createProductIndex);
	}
	...
	public List<BookDTO> retrieveBooksWithkeywordsFromCache(String keywords, Integer pageNo, Integer pageSize) {
		...
		n1qlQuery.parameterized(placeHolder, placeHolder);
